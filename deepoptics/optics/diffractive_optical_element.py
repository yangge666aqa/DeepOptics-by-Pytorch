...
from deepoptics.constants import MATERIAL_REFRACTIVE_INDEX_FUNCS
from deepoptics.constants import wave_length_list_400_700nm
import numpy
...
class RotationallySymmetricDOELayer(nn.Module):
    def __init__(self, image_patch=(512, 512), _quantization_level_cnt=4, BASE_PLANE_THICKNESS=2 * 1e-3,
                 quantization_height_base_wavelength=700 * 1e-9, height_tolerance=2e-8, glass_name='SK1300',
                 wave_length=wave_length_list_400_700nm, device='cuda'):
        super(RotationallySymmetricDOELayer, self).__init__()
        self.height_map_weight = nn.Parameter(torch.zeros(image_patch[0]), requires_grad=True)
        self.height_map_shape = nn.Parameter(torch.tensor(image_patch[0]), requires_grad=False)
        # self._quantization_level_cnt = _quantization_level_cnt
        # print("[QDO] Quantization levels：", _quantization_level_cnt)
        self.base_plane_thickness = nn.Parameter(torch.tensor(BASE_PLANE_THICKNESS), requires_grad=False)
        self.quantization_height_base_wavelength = nn.Parameter(torch.tensor(quantization_height_base_wavelength),
                                                                requires_grad=False)
        # self.wavelength_to_refractive_index_func = MATERIAL_REFRACTIVE_INDEX_FUNCS[glass_name]
        self.quantization_base_height = nn.Parameter(torch.tensor(_phase_to_height_with_material_refractive_idx_func(
            _phase=2 * torch.pi,  # 2*pi,可以调节以控制不同的刻蚀高度，如4pi对应2h_max刻的更深
            _wavelength=quantization_height_base_wavelength,
            _refractive_index_function=MATERIAL_REFRACTIVE_INDEX_FUNCS[glass_name]),
            dtype=torch.float32), requires_grad=False)  # Δn*h_max=λ (h_max in paper)
        self.radius_distance = nn.Parameter(create_Rotationally_radius_mesh(self.height_map_shape), requires_grad=False)
        self.wavelength = nn.Parameter(torch.from_numpy(wave_length), requires_grad=False)
        self.aperture = nn.Parameter(circular_aperture(self.height_map_shape),
                                     requires_grad=False)  # set the circular aperture
    ...
    def forward(self, Input_field, step, writer):
        preprocess_height = (self.height_map_weight + 1) * 0.5
        base_plane_height_map = self.base_plane_thickness - self.quantization_base_height * preprocess_height

        radius = self.height_map_shape
        radius_distance = self.radius_distance

        Rotationally_Height_map = torch.where((radius_distance >= 0) & (radius_distance <= 1), base_plane_height_map[0],
                                              0)
        for i in torch.arange(1, radius):
            Rotationally_Height_map += torch.where((radius_distance > i) & (radius_distance <= i + 1),
                                                   base_plane_height_map[i], 0)

        height_map = Rotationally_Height_map.view(1, 2 * radius, 2 * radius, 1)  # 整形为四维

        delta_n = MATERIAL_REFRACTIVE_INDEX_FUNCS['SK1300'](self.wavelength) - 1
        delta_n = delta_n.reshape((1, 1, 1, -1))
        k = 2. * torch.pi / self.wavelength
        k = k.reshape((1, 1, 1, -1))  # (1, 1, 1, 31)
        phi = torch.exp(1j * k * delta_n * height_map).type(torch.complex64)
        field_after_element = Input_field * phi
        field_after_element = field_after_element * self.aperture.view(1, 2 * self.height_map_shape,
                                                                       2 * self.height_map_shape, 1)
        if writer is not None and step % 30 == 0:
            prim_height = height_map.view(1, Input_field.shape[1], Input_field.shape[2])
            prim_height[prim_height > 1e-6] = self.base_plane_thickness - prim_height[prim_height > 1e-6]
            writer.add_image(tag='phase distribution',
                             img_tensor=prim_height / torch.max(prim_height)
                             , global_step=step)
        return field_after_element
